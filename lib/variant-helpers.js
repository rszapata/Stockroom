...
// Encuentra la variante de un item que corresponde a un cambio (vc) de un
// ajuste de tipo variant/source/all. Primero matchea por attrKey (clave
// normalizada de atributos); si falla — típicamente porque ML renombró el
// value_name de la variante y la attrKey quedó vieja — cae a matchear por el
// variation_id guardado al detectar el desbalance, que es ESTABLE aunque cambie
// el nombre. Sólo válido para ajustes variant/source/all (su varId es del MISMO
// item destino); NO usar para sale/cancel (ahí el varId sería de otra cuenta).
function _matchVarForApply(vars, vc) {
  let v = (vars || []).find(x => _varKeysAll(x).some(k => k === vc.attrKey));
  if (!v && vc.varId != null) v = (vars || []).find(x => x.id === vc.varId);
  return v || null;
}
...
module.exports = {
  _normalizeStr, _varKeysAll, _matchVarForApply, _varKeyFromOrderAttrs, _varLabelFromOrderAttrs, _varLabel, _fmtVarDelta, _shortAcct, _adjStaleMsg, _errMsg,
};
