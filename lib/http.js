...
function securityHeaders(res, isHtml, fileType, hasVersionQuery) {
  ...
  // COOP: en HTML usamos 'same-origin-allow-popups' para que el login con Google
  // (signInWithPopup de Firebase) pueda recibir el resultado del popup. 'same-origin'
  // a secas aísla el popup del opener y el login queda colgado sin retornar.
  res.setHeader('Cross-Origin-Opener-Policy', isHtml ? 'same-origin-allow-popups' : 'same-origin');
