...
function localProductoToItem(lp) {
  ...
  const out = {
    ...
    // Pago únicamente por transferencia (modelo costo+margen): la PDP no
    // muestra cuotas y el checkout oculta MP / no aplica el 5% de descuento.
    wz_solo_transferencia: !!lp.pago_transferencia,
  };
...
function _buildProductoPropioFields(data, { partial }) {
  ...
  if (!partial || has('pago_transferencia')) fields.pago_transferencia = !!data.pago_transferencia;
  ...
  if (!partial || has('variantes')) {
    ...
    fields.variantes = arr.map((v, idx) => ({
      ...
      colores: Array.isArray(v.colores)
        ...
        : [],
      // Fotos propias de la variante (URLs del pool de fotos del producto o
      // subidas): la PDP muestra esta galería al seleccionarla. Vacío = usa
      // las fotos generales del producto.
      imagenes: Array.isArray(v.imagenes)
        ? v.imagenes.map(u => String(u).trim().slice(0, 1000))
            .filter(u => /^(https?:\/\/|\/uploads\/)/i.test(u)).slice(0, 10)
        : [],
...
function _serializeProductoPropio(lp) {
  return {
    ...
    pago_transferencia: !!lp.pago_transferencia,
