...
function emailConfirmacionOrden(orden) {
  ...
  const metodoLabel = { mercadopago: 'MercadoPago', transferencia: 'Transferencia bancaria' };
...
// ── Email: bienvenida al crear la cuenta ──────────────────────────
// Mail transaccional puramente informativo (NO verificación): se manda
// una sola vez, cuando se crea la cuenta. La cuenta ya está activa.
function emailBienvenidaCuenta({ nombre, email, provider }) {
  const c = EMAIL_COLORS;
  const metodo = provider === 'google.com'
    ? 'tu cuenta de Google'
    : 'tu email y contraseña';
  const saludo = nombre ? `Hola ${_escEmail(nombre.split(' ')[0])}` : 'Hola';

  const body = `
    <p style="margin:0 0 12px;font-size:11px;font-weight:700;letter-spacing:0.12em;text-transform:uppercase;color:${c.accent};">Cuenta creada</p>
    <h1 class="email-text" style="margin:0 0 12px;font-size:24px;line-height:1.3;font-weight:700;color:${c.text};">${saludo}, ¡bienvenido/a a WZMALLAS!</h1>
    <p class="email-text-2" style="margin:0 0 28px;font-size:15px;line-height:1.6;color:${c.text2};">
      Tu cuenta ya está lista. La creaste usando <strong class="email-text" style="color:${c.text};">${metodo}</strong>${email ? ` (<span class="email-text">${_escEmail(email)}</span>)` : ''}. Desde tu cuenta vas a poder seguir tus pedidos y guardar tus datos para comprar más rápido.
    </p>

    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin-top:8px;">
      <tr>
        <td align="center">
          <a href="https://wzmallas.com/tienda/mi-cuenta.html" style="display:inline-block;background:${c.accent};color:#FFFFFF;font-size:14px;font-weight:600;padding:14px 32px;border-radius:10px;text-decoration:none;">Ir a mi cuenta</a>
        </td>
      </tr>
    </table>

    <p class="email-text-3" style="margin:28px 0 0;font-size:13px;line-height:1.6;color:${c.text3};text-align:center;">Si no fuiste vos quien creó esta cuenta, escribinos por <a class="email-text-3" href="https://wa.me/5492304216009" style="color:${c.text3};text-decoration:underline;">WhatsApp</a> o respondé este correo.</p>
  `;

  return _emailShell({
    preheader: 'Tu cuenta de WZMALLAS ya está lista.',
    bodyHtml: body,
  });
}

// Aviso "volvió el stock" — a los que dejaron su email en la PDP agotada (back-in-stock)
function emailBackInStock({ titulo, variant, url }) {
  const c = EMAIL_COLORS;
  const varTxt = variant ? ` — <strong class="email-text" style="color:${c.text};">${_escEmail(variant)}</strong>` : '';
  const body = `
    <p style="margin:0 0 12px;font-size:11px;font-weight:700;letter-spacing:0.12em;text-transform:uppercase;color:${c.accent};">¡Volvió el stock!</p>
    <h1 class="email-text" style="margin:0 0 12px;font-size:24px;line-height:1.3;font-weight:700;color:${c.text};">Ya tenemos disponible lo que esperabas</h1>
    <p class="email-text-2" style="margin:0 0 24px;font-size:15px;line-height:1.6;color:${c.text2};">
      <strong class="email-text" style="color:${c.text};">${_escEmail(titulo)}</strong>${varTxt} volvió a estar en stock. Aprovechá antes de que se agote de nuevo.
    </p>

    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin-top:8px;">
      <tr>
        <td align="center">
          <a href="${url}" style="display:inline-block;background:${c.accent};color:#FFFFFF;font-size:14px;font-weight:600;padding:14px 32px;border-radius:10px;text-decoration:none;">Ver producto</a>
        </td>
      </tr>
    </table>

    <p class="email-text-3" style="margin:28px 0 0;font-size:13px;line-height:1.6;color:${c.text3};text-align:center;">Recibís este aviso porque pediste que te avisáramos cuando volviera el stock de este producto.</p>
  `;
  return _emailShell({
    preheader: `Volvió el stock de ${titulo}`,
    bodyHtml: body,
  });
}

// ── Gancho de favoritos: volvió el stock de un favorito ──────────
function emailFavBackInStock({ titulo, url }) {
  const c = EMAIL_COLORS;
  const body = `
    <p style="margin:0 0 12px;font-size:11px;font-weight:700;letter-spacing:0.12em;text-transform:uppercase;color:${c.accent};">Uno de tus favoritos</p>
    <h1 class="email-text" style="margin:0 0 12px;font-size:24px;line-height:1.3;font-weight:700;color:${c.text};">Volvió el stock de tu favorito</h1>
    <p class="email-text-2" style="margin:0 0 24px;font-size:15px;line-height:1.6;color:${c.text2};">
      <strong class="email-text" style="color:${c.text};">${_escEmail(titulo)}</strong> volvió a estar disponible. Lo guardaste en tus favoritos — aprovechá antes de que se agote de nuevo.
    </p>
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin-top:8px;">
      <tr><td align="center">
        <a href="${url}" style="display:inline-block;background:${c.accent};color:#FFFFFF;font-size:14px;font-weight:600;padding:14px 32px;border-radius:10px;text-decoration:none;">Ver producto</a>
    ...
    <p class="email-text-3" style="margin:28px 0 0;font-size:13px;line-height:1.6;color:${c.text3};text-align:center;">Recibís este aviso porque tenés este producto en tu lista de favoritos de WZMALLAS.</p>
  `;
  return _emailShell({ preheader: `Volvió el stock de ${titulo}`, bodyHtml: body });
}

// ── Gancho de favoritos: bajó el precio de un favorito ───────────
function emailFavPriceDrop({ titulo, url, precioViejo, precioNuevo }) {
  const c = EMAIL_COLORS;
  const fmt = n => '$' + Math.round(Number(n) || 0).toLocaleString('es-AR');
  const baja = (precioViejo && precioNuevo) ? Math.round((1 - precioNuevo / precioViejo) * 100) : null;
  const body = `
    <p style="margin:0 0 12px;font-size:11px;font-weight:700;letter-spacing:0.12em;text-transform:uppercase;color:${c.accent};">Uno de tus favoritos</p>
    <h1 class="email-text" style="margin:0 0 12px;font-size:24px;line-height:1.3;font-weight:700;color:${c.text};">Bajó de precio${baja ? ` −${baja}%` : ''}</h1>
    <p class="email-text-2" style="margin:0 0 20px;font-size:15px;line-height:1.6;color:${c.text2};">
      <strong class="email-text" style="color:${c.text};">${_escEmail(titulo)}</strong>, que tenés en favoritos, ahora está más barato:
    </p>
    <p style="margin:0 0 24px;font-size:15px;line-height:1.4;">
      <span class="email-text-3" style="color:${c.text3};text-decoration:line-through;">${fmt(precioViejo)}</span>
      &nbsp;→&nbsp;
      <strong style="color:${c.accent};font-size:22px;">${fmt(precioNuevo)}</strong>
    </p>
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin-top:8px;">
      <tr><td align="center">
        <a href="${url}" style="display:inline-block;background:${c.accent};color:#FFFFFF;font-size:14px;font-weight:600;padding:14px 32px;border-radius:10px;text-decoration:none;">Ver producto</a>
    ...
    <p class="email-text-3" style="margin:28px 0 0;font-size:13px;line-height:1.6;color:${c.text3};text-align:center;">Recibís este aviso porque tenés este producto en tu lista de favoritos de WZMALLAS.</p>
  `;
  return _emailShell({ preheader: `Bajó de precio: ${titulo}`, bodyHtml: body });
}

// ── Email: pedido de reseña post-entrega (FASE C) ────────────────
function emailPedirResena({ nombre, producto, url }) {
  const c = EMAIL_COLORS;
  const hola = nombre ? `Hola ${_escEmail(String(nombre).split(' ')[0])}, ` : '';
  const body = `
    <p style="margin:0 0 12px;font-size:11px;font-weight:700;letter-spacing:0.12em;text-transform:uppercase;color:${c.accent};">Tu opinión</p>
    <h1 class="email-text" style="margin:0 0 12px;font-size:24px;line-height:1.3;font-weight:700;color:${c.text};">¿Cómo te fue con tu compra?</h1>
    <p class="email-text-2" style="margin:0 0 20px;font-size:15px;line-height:1.6;color:${c.text2};">
      ${hola}esperamos que estés disfrutando ${producto ? '<strong class="email-text" style="color:'+c.text+';">'+_escEmail(producto)+'</strong>' : 'tu compra'}. Nos encantaría saber tu opinión — a otros compradores les ayuda un montón.
    </p>
    <p class="email-text-2" style="margin:0 0 24px;font-size:15px;line-height:1.6;color:${c.text2};">
      Y si sumás una <strong class="email-text" style="color:${c.text};">foto</strong>, te regalamos un <strong style="color:${c.accent};">10% OFF</strong> para tu próxima compra.
    </p>
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin-top:8px;">
      <tr><td align="center">
        <a href="${url}" style="display:inline-block;background:${c.accent};color:#FFFFFF;font-size:14px;font-weight:600;padding:14px 32px;border-radius:10px;text-decoration:none;">Dejar mi reseña</a>
      </td></tr>
    </table>
    <p class="email-text-3" style="margin:28px 0 0;font-size:13px;line-height:1.6;color:${c.text3};text-align:center;">Recibís este correo porque hiciste una compra en WZMALLAS. ¡Gracias por elegirnos!</p>
  `;
  return _emailShell({ preheader: `¿Cómo te fue? Dejá tu reseña y ganá 10% OFF`, bodyHtml: body });
}

// ── Email: campaña de newsletter (FASE G) ───────────────────────
// cuerpo = texto plano del dueño (se escapa y se parte en párrafos). CTA y
// link de baja opcionales/obligatorio. NO se confía en HTML crudo del input.
function emailCampania({ titulo, cuerpo, ctaLabel, ctaUrl, unsubUrl }) {
  const c = EMAIL_COLORS;
  const parrafos = String(cuerpo || '').split(/\n\s*\n/).map(p =>
    `<p class="email-text-2" style="margin:0 0 16px;font-size:15px;line-height:1.6;color:${c.text2};">${_escEmail(p).replace(/\n/g, '<br>')}</p>`
  ).join('');
  const cta = (ctaUrl && ctaLabel) ? `
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:8px 0 4px;">
      <tr><td align="center">
        <a href="${_escEmail(ctaUrl)}" style="display:inline-block;background:${c.accent};color:#FFFFFF;font-size:14px;font-weight:600;padding:14px 32px;border-radius:10px;text-decoration:none;">${_escEmail(ctaLabel)}</a>
      </td></tr>
    </table>` : '';
  const body = `
    ${titulo ? `<h1 class="email-text" style="margin:0 0 16px;font-size:23px;line-height:1.3;font-weight:700;color:${c.text};">${_escEmail(titulo)}</h1>` : ''}
    ${parrafos}
    ${cta}
    <p class="email-text-3" style="margin:28px 0 0;font-size:12px;line-height:1.6;color:${c.text3};text-align:center;">
      Recibís este correo porque te suscribiste a las novedades de WZMALLAS.
      ${unsubUrl ? `<br><a href="${_escEmail(unsubUrl)}" style="color:${c.text3};text-decoration:underline;">Darme de baja</a>` : ''}
    </p>`;
  return _emailShell({ preheader: titulo || 'Novedades de WZMALLAS', bodyHtml: body });
}

// ── Bienvenida al newsletter: cupón de regalo por suscribirse ──────
function emailNewsletterBienvenida({ cupon, offPct = 10, dias = 30 }) {
  const c = EMAIL_COLORS;
  const body = `
    <p style="margin:0 0 12px;font-size:11px;font-weight:700;letter-spacing:0.12em;text-transform:uppercase;color:${c.accent};">Bienvenido a WZMALLAS</p>
    <h1 class="email-text" style="margin:0 0 12px;font-size:24px;line-height:1.3;font-weight:700;color:${c.text};">Tu ${offPct}% OFF de bienvenida</h1>
    <p class="email-text-2" style="margin:0 0 24px;font-size:15px;line-height:1.6;color:${c.text2};">
      Gracias por sumarte. Este cupón es tuyo: usalo en el checkout y llevate un <strong class="email-text" style="color:${c.text};">${offPct}% de descuento</strong> en tu próxima compra.
    </p>

    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:8px 0 24px;">
      <tr>
        <td align="center">
          <div style="display:inline-block;border:2px dashed ${c.accent};border-radius:10px;padding:12px 28px;font-family:'Courier New',monospace;font-size:22px;font-weight:700;letter-spacing:2px;color:${c.accent};">${_escEmail(cupon)}</div>
        </td>
      </tr>
    </table>
    ...
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
      <tr>
        <td align="center">
          <a href="https://wzmallas.com/tienda/catalogo.html?utm_source=newsletter&utm_campaign=bienvenida" style="display:inline-block;background:${c.accent};color:#FFFFFF;font-size:14px;font-weight:600;padding:14px 32px;border-radius:10px;text-decoration:none;">Ver el catálogo</a>
        </td>
      </tr>
    </table>

    <p class="email-text-3" style="margin:28px 0 0;font-size:13px;line-height:1.6;color:${c.text3};text-align:center;">Válido por ${dias} días. Recibís este correo porque te suscribiste al newsletter en wzmallas.com.</p>
  `;
  return _emailShell({
    preheader: `Tu cupón de ${offPct}% OFF de bienvenida: ${cupon}`,
    bodyHtml: body,
  });
}
...
module.exports = {
  ...
  emailCarritoAbandonado, emailBienvenidaCuenta, emailBackInStock,
  emailFavBackInStock, emailFavPriceDrop, emailPedirResena, emailCampania,
  emailNewsletterBienvenida,
};
