...
function createMlClient({ mlGet, mlPut, mlPost, mlOauthToken, atomicWriteFileSync, CONFIG_PATH, getConfig, getFullConfig }) {
  ...
  async function mlPutVerified(acct, itemId, putBody, expectedTotal) {
    ...
    // Conflicto TRANSITORIO de ML (KVS): escrituras concurrentes al mismo/otro
    // item devuelven 409 Conflict o 400 "TransactionConflict". No es un rechazo
    // real — hay que reintentar (el PUT setea cantidades absolutas, es idempotente).
    const isConflict = (e) => {
      const msg = String((e && e.message) || '');
      return e && (e.status === 409 || (e.status === 400 && /conflict/i.test(msg)) || /TransactionConflict|conflict\.error/i.test(msg));
    };
    ...
    for (let attempt = 0; ; attempt++) {
      try {
        ...
        if (isConflict(e) && attempt < 4) {
          // backoff con jitter para que dos PUT en conflicto no vuelvan a chocar
          await new Promise(r => setTimeout(r, 350 * (attempt + 1) + Math.floor(Math.random() * 250)));
          continue;
        }
