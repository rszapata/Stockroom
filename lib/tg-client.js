...
function createTgClient({ getFullConfig, telegramDisabled }) {
  ...
  function tgEditCaption(chatId, messageId, caption, keyboard) {
    ...
    // Devuelve la respuesta si Telegram confirmó (ok:true), o null si falló
    // (error de red o ok:false) — así el caller puede caer a un mensaje nuevo.
    return tgRequest(tg.bot_token, 'editMessageCaption', params)
  ...
  function tgEdit(chatId, messageId, text, keyboard) {
    ...
    // Devuelve la respuesta si Telegram confirmó (ok:true), o null si falló
    // (error de red o ok:false) — así el caller puede caer a un mensaje nuevo.
    return tgRequest(tg.bot_token, 'editMessageText', params)
      .then(r => (r && r.ok) ? r : null).catch(() => null);
  }
